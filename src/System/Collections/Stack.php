...
require_once(dirname(__FILE__) . "/generic/StackEnumerator.php");
 ...
 * @subpackage Collections
  */
class Stack implements ICollection, \System\ICloneable {
    ...
    private function constructFromNumber($initialCapacity) {
        if($initialCapacity < 0) throw new \System\ArgumentOutOfRangeException("initialCapacity is less than zero.");
    ...
    {
        if(is_null($array)) throw new \System\ArgumentNullException("array is null.");
        if($index < 0 || $index > $this->count()) throw new \System\ArgumentOutOfRangeException("index is less than zero. -or- index greater than size of queue");
    ...
    public function getEnumerator() {
        return new \System\Collections\Generic\StackEnumerator($this);
    }
    ...
    public function peek() {
        if($this->count() == 0) throw new \System\InvalidOperationException("The System.Collections.Stack is empty.");
