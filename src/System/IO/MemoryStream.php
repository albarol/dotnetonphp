...
class MemoryStream extends Stream {
    ...
    public function __construct($buffer=array(), $index=null, $count=null, $writable=true) {
        ...
        if($index < 0 || $count < 0) throw new ArgumentOutOfRangeException("index or count are negative.");
    ...
    private function createMemory($buffer, $index, $count) {
        if(is_null($index)) {
            ...
            if(($finalIndex) > sizeof($buffer)) throw new ArgumentException("The sum of index and count is greater than the length of buffer.");
