...
class FileStream extends Stream {
     ...
     */
    public function __construct($path, $mode=FileMode::Open, $access=FileAccess::Read) {
        if(is_null($path)) throw new ArgumentNullException("path is null.");
        if(strlen($path) == 0) throw new ArgumentException('path is an empty string (""), contains only white space, or contains one or more invalid characters.');
        if(($mode == FileMode::Open) && !file_exists($path)) throw new FileNotFoundException("The file cannot be found, such as when mode is FileMode.Truncate or FileMode.Open, and the file specified by path does not exist.");
